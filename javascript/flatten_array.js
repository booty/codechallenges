...
should only be done if the current depth of nesting is less 
than n. The depth of the elements in the first array are 
considered to be 0.
...
Example 1:
  ...
  This is because the smallest possible depth of a subarray (0) is 
  not less than n=0. Thus, no subarray should be flattened. 
...
Example 2:
  ...
  Explanation
  The subarrays starting with 4, 7, and 13 are all flattened. This 
  is because their depth of 0 is less than 1. However [9, 10, 11] 
 ...
 * @return {any[]}
 */
var flat_recursive = function (arr, depth) {
  let result = [];
  for (let i = 0; i < arr.length; i++) {
    if (Array.isArray(arr[i]) && depth > 0) {
      result = result.concat(flat_recursive(arr[i], depth - 1));
    } else {
      result.push(arr[i]);
    }
  }
...
var flat_iterative_queue = function (arr, n) {
