...
class Solutions
  ...
  # def self.bizness(nums)
  #   sorted = nums.sort
  #   putsif "sorted:#{sorted}"
  #   results = []

  #   0.upto(nums.length - 1) do |a|
  #     a_val = sorted[a]
  #     break if a_val.positive?

  #     (nums.length - 1).downto(a + 2) do |c|
  #       c_val = sorted[c]
  #       ac_sum = a_val + c_val

  #       putsif("a:#{a}(#{a_val}), c:#{c}(#{c_val}), ac_sum:#{ac_sum}")
  ...
  #         foo = [a_val, b_val, c_val]
  #         foo_sum = foo.sum
  #         putsif "  #{foo} b:#{b}(#{b_val}) foo_sum:#{foo_sum}"

  #         break if foo_sum.negative?

  #         results << foo if foo_sum.zero?
  #       end
  #     end
  #   end
